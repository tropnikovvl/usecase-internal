...
skip_list = ["prometheus.json", "prometheus-remote-write.json"]
...
def write_group_to_file(resource_name, content, url, destination):
    ...
    # initialize header
    lines = header % {"name": resource_name, "url": url, "condition": cond.strip()}
...
def main():
    ...
    for chart in charts:
        ...
        if chart["type"] == "yaml":
            ...
            if flat_structure:
                resource = chart.get(
                    "name", path.basename(chart["source"]).replace(".json", "")
                )
                write_group_to_file(
                    resource,
